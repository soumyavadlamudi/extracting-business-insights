...
def visual_summary(type_, df, col):
    ...
    if type_ == 'hist':
        ...
        print("call the function with proper parameters")
...
def measure_of_dispersion(type_, df, col):
    ...
    if type_ == 'cv':
        ...
        disp = ((df[col] - df[col].mean())**2).sum()/len(df)
    elif type_ == 'range':
        disp = df[col].max() - df[col].min()
    elif type_ == 'mad':
        mean = df[col].mean()
        disp = abs(df[col] - mean).sum()/len(df)
    elif type_ == 'iqr':
        disp = df[col].quantile(q = 0.75) - df[col.quantile(q = 0.25)]
    elif type_ == 'cov':
        new = df[col]
...
def calculate_correlation(type_, df, col1, col2):
    ...
    newdf = df[[col1 , col2]]
    if type_ == 'Pearson':
        cov = newdf.cov().iloc[0,1]
        corr = cov / (df[col1].std() * df[col2].std())
    elif type_ == 'Spearman':
...
def event_independence_check(prob_event1, prob_event2, prob_event1_event2):
    ...
    if(prob_event1_event2 == prob_event1 * prob_event2):
        print("Events are independant")
    else:
        print("Events are dependant")
...
for count in list(df['country'].unique()):
    print('Country: ', count)
    visual_summary('scatter', df[df['country'] == count], ['year', 'inflation_annual_cpi'])
...
prob_crisis = {}
for cont in list(df['country'].unique()):
    print("Country ", cont)
    prob_crisis[cont] = round(calculate_probability_discrete(df[df['country'] == cont]['banking_crisis'], 'crisis'), 4)
    print("Probability of Crisis: ", prob_crisis[cont])
